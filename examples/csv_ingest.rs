...
use jetgraph_client::{
    Client, PropertyEntry, TransactionEdge, TransactionNode, TransactionNodeRef,
};
...
const COUNTRY_CODES: &[&str] = &["US", "GB", "DE", "FR", "CA", "AU", "JP", "SG", "NL", "SE"];
    ...
    fn parse() -> Self {
        ...
        while i < raw.len() {
            match raw[i].as_str() {
                "--endpoint" => {
                    i += 1;
                    args.endpoint = raw[i].clone();
                }
                "--file" => {
                    i += 1;
                    args.file = raw[i].clone();
                }
                "--workers" => {
                    i += 1;
                    args.workers = raw[i].parse().unwrap_or(32);
                }
                "--batch-size" => {
                    i += 1;
                    args.batch_size = raw[i].parse().unwrap_or(10_000);
                }
                "--limit" => {
                    i += 1;
                    args.limit = raw[i].parse().unwrap_or(0);
                }
                "--help" | "-h" => {
...
impl Counter {
    ...
    fn in_flight(&self) -> u64 {
        self.sent().saturating_sub(self.ack() + self.err())
    }
}
...
fn build_batch(
    ...
    for (i, (f, geo)) in rows.iter().zip(geo_results.iter()).enumerate() {
        ...
        e1.numeric_value = Some(1.0);
        e1.event_ts_secs = Some(ts);

        let mut e2 = TransactionEdge::new("OWNS_DEVICE", customer_ref, device_ref.clone());
        e2.numeric_value = Some(1.0);
        e2.event_ts_secs = Some(ts);

        let mut e3 = TransactionEdge::new("HAS_BIN", card_ref.clone(), bin_ref);
        e3.numeric_value = Some(1.0);
        e3.event_ts_secs = Some(ts);

        let mut e4 = TransactionEdge::new("TRANSACTS_AT", card_ref.clone(), merchant_ref);
        e4.numeric_value = Some(f.amount as f32);
        e4.event_ts_secs = Some(ts);

        let mut e5 = TransactionEdge::new("USES_DEVICE", card_ref.clone(), device_ref);
        e5.numeric_value = Some(f.amount as f32);
        e5.event_ts_secs = Some(ts);

        let mut e6 = TransactionEdge::new("USES_IP", card_ref, ip_ref);
        e6.numeric_value = Some(f.amount as f32);
        e6.event_ts_secs = Some(ts);
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for w in 0..args.workers {
        ...
        worker_handles.push(tokio::spawn(async move {
            ...
            let (stream_tx, stream_rx) = match client.ingest_stream().await {
                Ok(pair) => pair,
                Err(e) => {
                    eprintln!("⚠ worker {w}: failed to open stream: {e}");
                    return;
                }
            };
                ...
                while let Some(result) = responses.next().await {
                    match result {
                        Ok(resp) => {
                            ...
                            nc2.err
                                .fetch_add(resp.node_errors as u64, Ordering::Relaxed);
                            ec2.ack.fetch_add(
                                (resp.edges_created + resp.edges_updated) as u64,
                                Ordering::Relaxed,
                            );
                            ec2.err
                                .fetch_add(resp.edge_errors as u64, Ordering::Relaxed);
                        }
                        Err(e) => {
            ...
            loop {
                ...
                if let Err(e) = stream_tx
                    .send(Some(&task.transaction_id), &task.nodes, &task.edges)
                    .await
                {
                    ...
                    if tc.err() <= 5 {
                        eprintln!("⚠ worker {w}: send error: {e}");
                    }
                    break;
                }
            }
    ...
    loop {
        // ── Parse one CSV row ─────────────────────────────────────────────────
        if args.limit != 0 && global_row >= args.limit as u64 {
            break;
        }
    ...
    println!(
        "  CSV rows/s       : {:.0}",
        global_row as f64 / total_s.max(0.001)
    );
    println!(
        "  Rows/request     : {:.1}",
