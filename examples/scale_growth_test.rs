    ...
    fn parse() -> Result<Self, String> {
        ...
        while i < args.len() {
            match args[i].as_str() {
                ...
                "--endpoint" => {
                    i += 1;
                    cfg.endpoint = val(&args, i, "--endpoint")?;
                }
                "--user-count" => {
                    i += 1;
                    cfg.user_count = pu(&args, i, "--user-count")?;
                }
                "--product-count" => {
                    i += 1;
                    cfg.product_count = pu(&args, i, "--product-count")?;
                }
                "--device-count" => {
                    i += 1;
                    cfg.device_count = pu(&args, i, "--device-count")?;
                }
                "--edge-count" => {
                    i += 1;
                    cfg.edge_count = pu(&args, i, "--edge-count")?;
                }
                "--checkpoint-every" => {
                    i += 1;
                    cfg.checkpoint_every = pu(&args, i, "--checkpoint-every")?;
                }
                "--concurrency" => {
                    i += 1;
                    cfg.concurrency = pu(&args, i, "--concurrency")?;
                }
                "--probe-iters" => {
                    i += 1;
                    cfg.probe_iters = pu(&args, i, "--probe-iters")?;
                }
                "--bootstrap-schema" => {
...
fn pu(args: &[String], i: usize, f: &str) -> Result<usize, String> {
    val(args, i, f)?
        .parse::<usize>()
        .map_err(|_| format!("invalid usize for {f}"))
}

fn print_help() {
    println!(
        "Usage: cargo run --release --example scale_growth_test -- [options]
...
impl Rng {
    ...
    fn bool_pct(&mut self, pct: u64) -> bool {
        self.next() % 100 < pct
    }
}
...
impl QueryProbeResult {
    fn avg_ms(&self) -> f64 {
        avg_micros(&self.latencies_us) / 1000.0
    }
    fn p50_ms(&self) -> f64 {
        let mut s = self.latencies_us.clone();
        s.sort_unstable();
        percentile(&s, 50.0) / 1000.0
    }
    fn p95_ms(&self) -> f64 {
        let mut s = self.latencies_us.clone();
        s.sort_unstable();
        percentile(&s, 95.0) / 1000.0
    }
    fn p99_ms(&self) -> f64 {
        let mut s = self.latencies_us.clone();
        s.sort_unstable();
        percentile(&s, 99.0) / 1000.0
    }
    fn max_ms(&self) -> f64 {
        self.latencies_us.iter().copied().max().unwrap_or(0) as f64 / 1000.0
    }
    fn is_degraded(&self) -> bool {
        self.p99_ms() > 10.0 || (self.errors as f64 / self.iters.max(1) as f64) > 0.01
    }
}
...
impl Checkpoint {
    fn interval_ops_s(&self) -> f64 {
        if self.interval_secs < 0.001 {
            0.0
        } else {
            self.interval_ops as f64 / self.interval_secs
        }
    }
}
...
async fn bootstrap_schema(client: &Client) -> Result<(), Box<dyn std::error::Error>> {
    ...
    if needs_user {
        s.register_node_type("user", true).await?;
        println!("    + node type: user");
    }
    if needs_product {
        s.register_node_type("product", true).await?;
        println!("    + node type: product");
    }
    if needs_device {
        s.register_node_type("device", true).await?;
        println!("    + node type: device");
    }
    ...
    for _ in 0..concurrency {
        ...
        handles.push(tokio::spawn(async move {
            ...
            loop {
                ...
                if tx
                    .send(None, &[TransactionNode::new(node_type, &ext)], &[])
                    .await
                    .is_err()
                {
                    break;
...
async fn ingest_user_chunk(
        ...
        handles.push(tokio::spawn(async move {
            ...
            let reader = tokio::spawn(async move {
                ...
                while let Some(Ok(resp)) = responses.next().await {
                    stats2
                        .nodes_written
                        .fetch_add(resp.nodes_created as usize, Ordering::Relaxed);
            ...
            loop {
                ...
                if rng.bool_pct(50) {
                    let mut e =
                        TransactionEdge::new("PURCHASES", user_ref.clone(), product_ref.clone());
    ...
    Ok(Arc::try_unwrap(stats).unwrap_or_else(|a| ChunkStats {
...
async fn run_query_probes(
    ...
    let r1 = probe("edge_state", iters, |i| {
        ...
        async move {
            ...
            .await
            .map(|_| ())
            .map_err(|e| format!("user={uid} product={pid}: {e}"))
        }
    })
    .await;
    ...
    let r4 = probe("neighbors_in_50", iters, |i| {
        ...
        async move {
            ...
            .await
            .map(|_| ())
            .map_err(|e| format!("product={pid}: {e}"))
        }
    })
    .await;
    ...
    let r5 = probe("two_hop", iters, |i| {
        ...
        async move {
            ...
            .await
            .map(|_| ())
            .map_err(|e| format!("hop2 product_node={first_product_id}: {e}"))
        }
    })
    .await;
...
fn print_checkpoint(cp: &Checkpoint) {
    println!("\n╔══════════════════════════════════════════════════════════════════════════════╗");
    println!(
        "  CHECKPOINT #{:<3} │ nodes={:<12} │ edges={:<12} │ elapsed={:.1}s",
        cp.index,
...
fn print_summary(checkpoints: &[Checkpoint]) {
    ...
    println!(
        "  {:<4}  {:<14} {:<12} {:<10}  {:>10}  {}",
        "#", "nodes", "edges", "ingest/s", "edge_state p99", "two_hop p99"
    );
    println!("  {}", "─".repeat(90));

    for cp in checkpoints {
        let edge_state_p99 = cp
            .queries
            .iter()
    ...
    if degraded.is_empty() {
        ...
        for bp in degraded {
            println!(
                ...
                bp.index,
                fmt_n(bp.total_nodes),
                fmt_n(bp.total_edges)
            );
            for q in &bp.queries {
                if q.is_degraded() {
                    println!(
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "Phase 1 — Loading {} PRODUCT nodes...",
        fmt_n(cfg.product_count)
    );
    let (prod_created, prod_elapsed) =
        preload_nodes(&client, "product", cfg.product_count, cfg.concurrency).await?;
    println!(
        "  Done: {} created in {:.2}s ({:.0} nodes/s)\n",
        fmt_n(prod_created),
        prod_elapsed.as_secs_f64(),
        prod_created as f64 / prod_elapsed.as_secs_f64().max(0.001)
    );

    // ── Phase 2: Pre-load DEVICE nodes ────────────────────────────────────────
    println!(
        "Phase 2 — Loading {} DEVICE nodes...",
        fmt_n(cfg.device_count)
    );
    let (dev_created, dev_elapsed) =
        preload_nodes(&client, "device", cfg.device_count, cfg.concurrency).await?;
    println!(
        "  Done: {} created in {:.2}s ({:.0} nodes/s)\n",
        fmt_n(dev_created),
        dev_elapsed.as_secs_f64(),
        dev_created as f64 / dev_elapsed.as_secs_f64().max(0.001)
    );

    // ── Phase 3: USER nodes + edges, with periodic checkpoints ───────────────
    println!(
        "Phase 3 — Loading {} USER nodes with edges...",
        fmt_n(cfg.user_count)
    );
    println!(
        "  Will checkpoint every {} total records.\n",
    ...
    while user_cursor < cfg.user_count && edges_remaining > 0 {
        ...
        // ── checkpoint? ───────────────────────────────────────────────────────
        if cumulative_records >= next_checkpoint
            || user_cursor >= cfg.user_count
            || edges_remaining == 0
        {
            checkpoint_idx += 1;
            ...
            let queries =
                run_query_probes(&client, cfg.probe_iters, users_known, products_known).await;
